...
public class MentorDetailsActivity extends Activity implements Callback, Button.OnClickListener {
    ...
    private void initialize() {
        ...
        String searchedCategoryId = getIntent().getStringExtra("searched_keyword");

        if (searchedCategoryId != null && !searchedCategoryId.equals("-1")) {
            searchedCategoryId = DataBase.singleton(this).getSubCategory(searchedCategoryId).trim();

            List<String> subCat = new ArrayList<String>();
            subCat.add(searchedCategoryId);
            userInfo.setSubCategoryName(subCat);
        }
